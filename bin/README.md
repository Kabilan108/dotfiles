...
- [sessionizer](./sessionizer):
  fleet-aware tmux session manager and project directory jumper with an fzf picker. It lists sessions and unused project directories from reachable machines. Remote selections replace the local tmux client over SSH and can return to the origin machine's picker without mirroring tmux state.
