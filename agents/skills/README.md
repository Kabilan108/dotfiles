...
A skill that only works on some fleet machines lists them in its frontmatter; without the key it installs everywhere:

```yaml
metadata:
  fleet-hosts: sietch            # or "sietch,jacurutu"
```
