...
```sh
agent-run start --provider claude --model claude-fable-5-1 --effort medium --access auto --repo /absolute/repo --prompt /absolute/review.txt
```

Run `agent-run --help` for lifecycle details. Immediately attach a managed wait as described in your global instructions. Read-only intent belongs in the prompt; the provider's access mode is separate. Claude auto mode uses automatic approval review; it is not a read-only sandbox. Do not silently bypass permissions for a review.
